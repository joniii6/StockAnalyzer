...
The first three functions, index, ytd, and value_today, retrieve and process financial data for a specified set of tickers. The index function retrieves year-to-date (YTD) data for the tickers from Yahoo Finance, calculates the percentage change in the 'Close' column from the first non-NaN value, and returns a DataFrame with the modified data. The ytd function calls the index function to retrieve the modified data, then calculates the YTD return for each ticker by subtracting the first value in the 'Close' column from the last value and returns a DataFrame with the YTD returns. The value_today function retrieves the YTD 'Close' data from Yahoo Finance, calculates the current value for each ticker by using the last non-NaN value in the 'Close' column, and returns a DataFrame with the current values.
...
Furthermore we created a code which enabled our webapp users to compare different tickers to each other. The code first obtains the tickers for various indices and their corresponding names. It then allows users to select some indices from a list of names using a multi-select widget.
