...
elif len(stocks_tickers.split(' ')) < 2 :
	st.error("Enter at least two valid tickers properly seperated with a space")
else:	
	try:
		for i in stocks_ticker_list:
			stocks_names.append(yf.Ticker(i).info["longName"])
	except:
		st.error("At least one of the Tickers is not valid or you have not properly seperated them with a space.")
	try:
		...
		ax1.set_title('YtD stock returns for your selected portfolio')
...
st.caption("The optimal portfolio is constructed by choosing the optimal weights for the individual stocks to maximize the sharpe ratio (excess return / risk taken).")

Investment_Amount = st.number_input("Select how much money you want to invest in your portfolio", min_value = 1000, step = 1000)

try:
	portfolio = yf.download(stocks_tickers, period = '5y')['Close']
